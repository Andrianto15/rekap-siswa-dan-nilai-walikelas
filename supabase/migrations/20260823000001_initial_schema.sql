...
-- Description: Complete initial schema, triggers, RLS policies, and seed master data
...
create policy "Tahun ajaran viewable by authenticated"
  on public.tahun_ajaran for select to authenticated using (true);

create policy "Tahun ajaran manageable by admin"
  on public.tahun_ajaran for all to authenticated
  using (public.is_admin()) with check (public.is_admin());

create policy "Semester viewable by authenticated"
  on public.semester for select to authenticated using (true);

create policy "Semester manageable by admin"
  on public.semester for all to authenticated
  using (public.is_admin()) with check (public.is_admin());
...
create policy "Mapel viewable by authenticated"
  on public.mapel for select to authenticated using (true);

create policy "Mapel manageable by admin"
  on public.mapel for all to authenticated
  using (public.is_admin()) with check (public.is_admin());

create policy "Guru kelas viewable by authenticated"
  on public.guru_kelas for select to authenticated using (true);

create policy "Guru kelas manageable by admin"
  on public.guru_kelas for all to authenticated
  using (public.is_admin()) with check (public.is_admin());

create policy "Guru mapel viewable by authenticated"
  on public.guru_mapel for select to authenticated using (true);

create policy "Guru mapel manageable by admin"
  on public.guru_mapel for all to authenticated
  using (public.is_admin()) with check (public.is_admin());
...
create policy "Kehadiran viewable by authenticated"
  on public.kehadiran for select to authenticated using (true);
...
create policy "Komponen nilai viewable by authenticated"
  on public.komponen_nilai for select to authenticated using (true);

create policy "Komponen nilai manageable by admin or guru mapel"
  on public.komponen_nilai for all to authenticated
  using (
    public.is_admin() or
    guru_id = auth.uid() or
    exists (
...
create policy "Nilai viewable by authenticated"
  on public.nilai for select to authenticated using (true);
...
create policy "Nilai akhir viewable by authenticated"
  on public.nilai_akhir for select to authenticated using (true);

create policy "Nilai akhir manageable by admin or guru"
  on public.nilai_akhir for all to authenticated
  using (
    public.is_admin() or
    guru_id = auth.uid() or
    exists (
